...
*/
    ...
    public ResponseEntity<ErrorInfo> updateOrCreate(@Valid UserMeal meal, BindingResult result, HttpServletRequest request) {
        if (result.hasErrors()) {
            // TODO change to exception handler
            StringBuilder sb = new StringBuilder();
            result.getFieldErrors().forEach(fe -> sb.append(fe.getField()).append(" ").append(fe.getDefaultMessage()).append("<br>"));
            return new ResponseEntity<>(validationError(request, new ValidationException(sb.toString())), HttpStatus.UNPROCESSABLE_ENTITY);
        }
        if (meal.isNew()) {
            super.create(meal);
        } else {
